...
应用投放设置保存在 `%LOCALAPPDATA%\MoniHop\config\application-projection.json`，窗口投放层设置保存在 `%LOCALAPPDATA%\MoniHop\config\window-projection.json`。普通权限无法移动更高权限窗口时，MoniHop 保留窗口原状态，不要求管理员常驻。
